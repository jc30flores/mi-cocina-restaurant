...
def table_links_collection():
    if request.method == 'GET':
        ...
        cur = conn.cursor(cursor_factory=RealDictCursor)
        try:
            cur.execute("SELECT group_id FROM linked_table_members WHERE table_number = %s;", (leader,))
            row = cur.fetchone()
            if row:
                gid = row['group_id']
            else:
                cur.execute("SELECT 1 FROM orders WHERE table_number = %s AND status != 'paid' LIMIT 1;", (leader,))
                if not cur.fetchone():
                    cur.close()
                    conn.close()
                    return jsonify({'error': 'no_active_order'}), 400
                cur.execute("INSERT INTO linked_table_groups DEFAULT VALUES RETURNING id;")
                gid = cur.fetchone()['id']
                cur.execute("INSERT INTO linked_table_members (group_id, table_number) VALUES (%s,%s);", (gid, leader))

            for t in tables:
                cur.execute("SELECT 1 FROM linked_table_members WHERE table_number = %s;", (t,))
                if cur.fetchone():
                    raise Exception('table_already_linked')
                cur.execute("SELECT 1 FROM orders WHERE table_number = %s AND status != 'paid' LIMIT 1;", (t,))
                if not cur.fetchone():
                    raise Exception('no_active_order')
...
@app.route('/api/unlink-table', methods=['POST'])
def unlink_table_endpoint():
    data = request.get_json() or {}
    table_number = data.get('table_number')
    unlink_all = bool(data.get('unlink_all'))
    ...
    try:
        ...
        if not row:
            cur.close()
            conn.close()
            return jsonify({'error': 'Table is not linked or already unlinked'}), 400
        gid = row['group_id']
        ...
        cur.execute(
            "SELECT 1 FROM orders WHERE table_number = ANY(%s) AND status = 'processing_payment' LIMIT 1;",
            (tables,)
        )
        if cur.fetchone():
            cur.close()
            conn.close()
            return (
                jsonify({'error': 'Unlinking is not allowed while payment is in progress.'}),
                403,
            )
        ...
        if unlink_all:
            ...
            cur.execute("DELETE FROM linked_table_members WHERE table_number = %s;", (table_number,))
            cur.execute(
                "SELECT COUNT(*) AS count FROM linked_table_members WHERE group_id = %s;",
                (gid,),
            )
        ...
        cur.close()
        conn.close()
        return jsonify({'error': 'Unlink failed', 'details': str(e)}), 500
